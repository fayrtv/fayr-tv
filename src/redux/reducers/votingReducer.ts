...
export const reducer = (state = initialState, action: ReducerAction<VotingData | AttendeeVote>): VotingReducerState => {
	switch (action.type) {
		case VOTING_CREATED:
			return [...state, action.payload as VotingData];
		case VOTING_UPDATE_VOTE:
			const attendeeVote = action.payload as AttendeeVote;
			const correspondingVoteIndex = state.findIndex(x => x.votingId === attendeeVote.votingId);
