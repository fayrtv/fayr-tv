...
class Controls extends Component {
	...
	handleChatClick = (e) => {
		...
		this.context.set(!this.context.isOpen);
	}
	...
	render() {
		...
		const chat_controls = this.context.isOpen ? 'controls__btn--chat_on' : 'controls__btn--chat_off';
		...
		return (
			<div className="controls pos-relative">
				...
				<button className="controls__btn btn rounded btn--leave btn--destruct" onClick={this.endButtonOnClick}>
					<svg className="btn__svg" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#ffffff" strokeWidth="2" strokeLinecap="square" strokeLinejoin="arcs"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
				</button>
				...
				<div className="btn--chat--container">
					...
					{!this.context.isOpen && this.props.unreadMessages.length > 0 && <span className="controls__btn btn--chat--unreadTag">{this.props.unreadMessages.length > 99 ? "99+" : this.props.unreadMessages.length}</span>}
				</div>
			</div>
		)
	}
