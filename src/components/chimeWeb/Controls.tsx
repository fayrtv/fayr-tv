...
const Controls: React.FC<Props & ReduxProps> = ({ chime, title, openSettings, unreadMessages, role, ssName, baseHref, history }) => {
	...
	const handleRoomClick: React.MouseEventHandler<HTMLButtonElement> = (e) => {
		e.stopPropagation();
		e.preventDefault();
	...
	const mic_controls = localMuted ? 'controls__btn--mic_on' : 'controls__btn--mic_off';
	const cam_controls = videoStatus === VideoStatus.Enabled ? 'controls__btn--cam_off' : 'controls__btn--cam_on';
	const chat_controls = isChatOpen ? 'controls__btn--chat_on' : 'controls__btn--chat_off';
	...
	return (
		<div className="controls pos-relative">
			...
			<button className={`controls__btn ${mic_controls} btn rounded btn--mic`} onClick={muteButtonOnClick}>
			...
			<button className={`controls__btn ${cam_controls} btn rounded btn--cam`} onClick={videoButtonOnClick}>
			...
			{/* Setting button */}
			<button className="controls__btn btn rounded btn--settings" onClick={openSettings}>
			...
			{/* End button */}
			<button className="controls__btn btn rounded btn--leave btn--destruct" onClick={endButtonOnClick}>
				<svg className="btn__svg" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#ffffff" strokeWidth="2" strokeLinecap="square" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
			</button>
			...
			{/* Watch Party button */}
			<button className="controls__btn btn rounded popup" onClick={handleRoomClick}>
			...
			<div className="btn--chat--container">
				<button className={`controls__btn ${chat_controls} btn rounded btn--chat`} onClick={handleChatClick} title={`${isChatOpen ? "Hide" : "Show"} chat`}>
				...
				{!isChatOpen && unreadMessages.length > 0 && <span className="controls__btn btn--chat--unreadTag">{unreadMessages.length > 99 ? "99+" : unreadMessages.length}</span>}
			</div>
		</div>
	)
}
