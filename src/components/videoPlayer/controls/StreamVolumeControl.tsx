...
export const StreamVolumeControl = ({ player }: Props) => {
	...
	const onMuteClick: React.MouseEventHandler = React.useCallback(event => {
	...
	const convertClickToVolume = React.useCallback((y: number, railRef: HTMLDivElement) => {
	...
	}, [muted, player, railRef, pinRef, isMobile, convertClickToVolume]);

	const { height } = railRef.current?.getBoundingClientRect() ?? { height: 0 };

	return (
		<div
			className={`mg-x-1 player-btn player-btn--icon ${muted ? "player-btn--mute" : "player-btn--unmute"} ${styles.StreamVolumeControl}`}
			onClick={onMuteClick}>
			<svg
